...
const STEPS = [
  ...
  },
  {
    ...
    subtitle:"Sensory sensitivities, things they hate, noise tolerance, anything that might help me get this exactly right.",
    type:"text",
...
// ─── STATIC PERSONALISATION (Option B, no live AI) ────────────────────────────
// Builds reasons that reference the child's chosen goals and interests,
// plus a friendly play idea, all without calling an external API.
const DEV_GOAL_WORDS={gross:"gross motor",fine:"fine motor",cognitive:"problem-solving",language:"language",social:"social and emotional",sensory:"sensory",selfcare:"self-care"};
const INTEREST_WORDS={animals:"animals",vehicles:"cars and trucks",dinosaurs:"dinosaurs",building:"building",
  "pretend-play":"pretend play",movement:"being active",art:"art and craft",music:"music",
  stories:"stories",puzzles:"puzzles",water:"water play",science:"how things work"};

function buildStaticReason(toy,answers){
  const goals=(answers.dev_focus||[]).filter(g=>g!=="no-preference");
  const interests=answers.interests||[];
  // Does this toy hit one of their goals?
  const matchedGoal=toy.dev_areas.find(a=>goals.includes(a));
  // Does it match an interest?
  const matchedInterest=(toy._matchedInterests||[]).find(i=>interests.includes(i));

  let lead="";
  if(matchedGoal){
    lead=`Great for the ${DEV_GOAL_WORDS[matchedGoal]} goal you mentioned. `;
  } else if(matchedInterest){
    lead=`Since they love ${INTEREST_WORDS[matchedInterest]||matchedInterest}, this is a natural fit. `;
  }
  return lead+toy.note;
}

function buildStaticPlayIdea(toy,answers){
  const interests=answers.interests||[];
  // Only add a play idea if the toy does NOT already match a main interest
  const matchesInterest=(toy._matchedInterests||[]).some(i=>interests.includes(i));
  if(matchesInterest)return "";
  // Suggest linking to a top interest they picked
  const topInterest=interests[0];
  if(!topInterest)return "";
  const w=INTEREST_WORDS[topInterest]||topInterest;
  const ideas={
    animals:`Bring their favourite animals into the play, maybe the toy becomes a home or hospital for them.`,
    vehicles:`Work their love of ${w} in, drive a truck over to it or make it part of a road or garage.`,
    dinosaurs:`Add a dinosaur or two, maybe the dinos need to use it, fix it, or hide near it.`,
    building:`Let them build something around it or combine it with their blocks.`,
    "pretend-play":`Turn it into a story, give the toy a character and a little adventure.`,
    movement:`Make it active, add a race, a jump, or an obstacle course to reach it.`,
    art:`Pair it with drawing, they could draw what they made or decorate alongside it.`,
    music:`Add a song or a beat while they play, music makes everything more engaging.`,
    stories:`Weave it into a story, what happens next for the characters?`,
    puzzles:`Turn it into a challenge, can they solve it a new way or beat their last try?`,
    water:`Bring it near water play if it's suitable, or add pouring and splashing alongside.`,
    science:`Ask what would happen if, let them experiment and test their ideas.`,
  };
  return ideas[topInterest]||`Bring their love of ${w} into the play to make it really click.`;
}

function buildStaticCuratorNote(answers){
  const age=ageLabel(answers.child_age);
  const goals=(answers.dev_focus||[]).filter(g=>g!=="no-preference").map(g=>DEV_GOAL_WORDS[g]);
  const interests=(answers.interests||[]).map(i=>INTEREST_WORDS[i]||i);
  let parts=[];
  parts.push(`At ${age}, play is how your child learns best, and these picks are chosen with that in mind.`);
  if(goals.length){
    const g=goals.length===1?goals[0]:goals.slice(0,-1).join(", ")+" and "+goals[goals.length-1];
    parts.push(`I've made sure there's something here for the ${g} you wanted to support.`);
  }
  if(interests.length){
    parts.push(`And I've leaned into what they love so the toys actually get played with, not left on the shelf.`);
  }
  parts.push(`Remember, every child develops at their own pace, and you know yours best.`);
  return parts.join(" ");
}
...
JSON only: {"reason":"..."}`;
    ...
    const productList=recs.map((p,i)=>`${i+1}. ${p.name} ($${p.price}) supports: ${p.dev_areas.map(d=>devLabelMap[d]||d).join(", ")}`).join("\n");
...
For toys that don't directly match the child's main interests, add a short play idea connecting them. Use Brittany's style from her 30 Day Play Calendar: specific, active, one sentence, a little playful. Examples of her style:
...
Keep play ideas brief, practical, and fun. Skip the play idea if the toy already directly matches their interests.
...
{"personalised_reasons":["r1","r2","r3","r4","r5"],"play_ideas":["idea or empty string","","","",""],"curator_note":"..."}`;
    ...
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1400,messages:[{role:"user",content:prompt}]})});
    const data=await res.json();
    return JSON.parse(data.content.map(b=>b.text||"").join("").replace(/```json|```/g,"").trim());
  }
  ...
  async function handleSubmit(){
    setLoading(true);
    // Option B: build personalised reasons locally, no external API needed.
    const recs=getRecommendations(answers,5,excludedIds);
    const personalised_reasons=recs.map(p=>buildStaticReason(p,answers));
    const play_ideas=recs.map(p=>buildStaticPlayIdea(p,answers));
    const curator_note=buildStaticCuratorNote(answers);
    // brief delay so the loading screen feels intentional
    setTimeout(()=>{
      setResult({recs,personalised_reasons,play_ideas,curator_note});
      setLoading(false);
    },650);
  }
  ...
  async function handleSwap(index){
    ...
    try{
      ...
      const reason=buildStaticReason(newToy,answers);
      const playIdea=buildStaticPlayIdea(newToy,answers);
      setResult(prev=>{
  ...
  if(loading)return(
    <div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"system-ui"}}>
      ...
      <div style={{fontSize:17,color:TEXT,fontWeight:700,marginBottom:6}}>Finding your picks...</div>
      <div style={{fontSize:14,color:MUTED}}>Matching toys to your child's age, interests and development</div>
